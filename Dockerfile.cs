...
ARG TARGETARCH
RUN --mount=type=secret,id=IRCX_SSPI_TOKEN \
    dotnet nuget add source "https://nuget.pkg.github.com/mono-chat/index.json" \
      --name "mono-chat" \
      --username jyonxo \
      --password "$(cat /run/secrets/IRCX_SSPI_TOKEN)" \
