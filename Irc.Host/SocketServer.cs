...
public class SocketServer : Socket, ISocketServer
{
    ...
    public SocketServer(IPAddress ip, int port, int backlog, int maxConnections, int maxConnectionsPerIp, int buffSize)
    ...
    public new void Listen()
    {
        ...
        acceptAsync.Completed += (_, args) => { AcceptLoop(args); };
    ...
    public void Accept(IConnection connection)
    {
        ...
        {
            connection.Disconnect("Too many connections");
            return;
        }
    ...
    private void ClientDisconnected(object? sender, BigInteger bigIP)
    {
        ...
        {
            Log.Info($"{sender}[{bigIP}] has disconnected but failed to remove from collection / total: {Sockets.Count} ");
            return;
        }
